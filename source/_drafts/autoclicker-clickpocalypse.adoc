...
Si vous comprenez le fonctionnement du couple `loop`/`next` présenté ici,
j'aurais transmis ce dont j'ai envie, la suite n'est que du bonus.
...
objet géant. Utilsées avec Redux par exemple, cela permet de limiter le store
...
* J'ai vonlontairement omis la gestion du contexte (`this`) pour l'appel
de la fonction. On peut le rajouter facilement si nécessaire.
